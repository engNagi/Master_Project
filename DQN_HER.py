...
class DQN(object):
    def __init__(self, action_n, scope,
        ...
        with tf.variable_scope(scope):
            ...
            self.obs = tf.placeholder(tf.float32, shape=(None, 512), name="image_features")
            self.ag_pos = tf.placeholder(tf.float32, shape=(None, 3), name="positions")

            # concatenate agent observations, and positions
            self.inputs = tf.concat((self.obs, self.ag_pos), axis=1, name="Inputs")
            self.goals_ = tf.placeholder(tf.float32, shape=(None, 3), name="Goals_")

            self.actions = tf.placeholder(tf.int32, shape=(None,), name='actions')

            #samples additional goals
    ...
    def predict(self, obs_state, pos_state, goal_):
        return self.session.run(self.predict_op, feed_dict={self.obs: obs_state,
                                                            self.ag_pos: pos_state,
                                                            self.goals_: goal_})
    ...
    def sample_action(self, obs_state, pos_state, goal_, eps):
        """Implements epsilon greedy algorithm"""
        if np.random.random() < eps:
            return np.random.choice(self.action_n)
        else:
            return np.argmax(self.predict([obs_state], [pos_state], [goal_])[0])
